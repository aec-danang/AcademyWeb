...
export default async function ClassroomHubPage({ params, searchParams }: Props) {
  ...
  const isActiveStudent = user.role === "STUDENT" && classroom.status === "ACTIVE" && classroom.enrollments.some((item: any) => item.userId === user.id && item.status === "ACTIVE");
  ...
  const activeEnrollments = classroom.enrollments.filter((item: any) => item.status === "ACTIVE");
  const pendingEnrollments = classroom.enrollments.filter((item: any) => item.status === "REQUESTED");
  const previousEnrollments = classroom.enrollments.filter((item: any) => !["ACTIVE", "REQUESTED"].includes(item.status));
  const allSubmissions = classroom.assignments.flatMap((item: any) => item.submissions);
  const pendingSubmissions = allSubmissions.filter((item: any) => item.status !== "GRADED" && !item.grade);
  const publishedAssignments = classroom.assignments.filter((item: any) => item.status === "PUBLISHED");
  const grades = allSubmissions.flatMap((item: any) => item.grade?.status === "PUBLISHED" && typeof item.grade.score === "number" ? [{ ...item.grade, score: item.grade.score, student: item.student }] : []);
  const averageScore = grades.length ? grades.reduce((sum: number, item: any) => sum + item.score, 0) / grades.length : null;
  ...
  const activityItems = [
    ...classroom.enrollments.map((item: any) => ({ key: `enrollment-${item.id}`, date: item.requestedAt, title: item.status === "REQUESTED" ? "Enrollment requested" : `Enrollment ${item.status.toLowerCase()}`, detail: item.student.name || item.student.email || "Student" })),
    ...classroom.assignments.map((item: any) => ({ key: `assignment-${item.id}`, date: item.updatedAt, title: `Assignment ${item.status.toLowerCase()}`, detail: item.title })),
    ...allSubmissions.map((item: any) => ({ key: `submission-${item.id}`, date: item.submittedAt, title: "Assignment submitted", detail: item.student.name || item.student.email || "Student" })),
  ].sort((a: any, b: any) => b.date.getTime() - a.date.getTime()).slice(0, 12);
    ...
    <div className={styles.classroomHub}>
      ...
      {activeTab === "overview" ? (
        <div className={styles.classroomHubContent}>
          ...
          <section className={`${styles.classroomSummaryGrid} ${styles.classroomDetailSummary}`}>
            ...
            <div><ClipboardList size={20} /><strong>{classroom.quizDeliveries.filter((item: any) => item.status === "PUBLISHED").length}</strong><span>Assigned quizzes</span></div>
      ...
      {activeTab === "students" && isManager ? (
        <div className={styles.classroomHubContent}>
          ...
          {pendingEnrollments.length ? <section className={styles.dashboardPanel}><div className={styles.dashboardPanelHeader}><div><span className={styles.cockpitEyebrow}><UserPlus size={16} /> Needs attention</span><h2>Pending enrollments</h2></div></div><div className={styles.classroomRoster}>{pendingEnrollments.map((item: any) => <div key={item.id}><div><strong>{item.student.name || "Unnamed student"}</strong><p>{item.student.email || "No email"} · Requested {formatDate(item.requestedAt)}</p></div><form action={decideEnrollmentAction}><input type="hidden" name="id" value={item.id} /><button name="decision" value="approve" className="btn-primary">Approve</button><button name="decision" value="reject" className="btn-secondary">Reject</button></form></div>)}</div></section> : null}
          <section className={styles.dashboardPanel}><div className={styles.dashboardPanelHeader}><div><span className={styles.cockpitEyebrow}><Users size={16} /> Roster</span><h2>Active students</h2></div></div>{activeEnrollments.length ? <div className={styles.classroomRoster}>{activeEnrollments.map((item: any) => <div key={item.id}><div><strong>{item.student.name || "Unnamed student"}</strong><p>{item.student.email || "No email"}</p></div><div className={styles.rosterActions}><span className={styles.scorePill}>Active</span><form action={removeStudentFromClassAction}><input type="hidden" name="enrollmentId" value={item.id} /><input type="hidden" name="classroomId" value={classroom.id} /><ConfirmSubmitButton className={styles.rosterRemoveButton} message={`Remove ${item.student.name || item.student.email || "this student"} from ${classroom.name}? Their account, submissions and score history will be kept.`}><UserMinus size={15} /> Remove</ConfirmSubmitButton></form></div></div>)}</div> : <div className={styles.classroomContextEmpty}><UserPlus size={28} /><strong>No students in this class yet</strong><p>Add an existing account above or share the class code.</p></div>}</section>
          {previousEnrollments.length ? <details className={styles.rosterHistory}><summary>Previous access ({previousEnrollments.length})</summary><div>{previousEnrollments.map((item: any) => <div key={item.id}><span><strong>{item.student.name || "Unnamed student"}</strong><small>{item.student.email || "No email"}</small></span><em>{item.status}</em></div>)}</div><p>Records remain here for audit. Add the same email again to restore classroom access.</p></details> : null}
        </div>
      ) : null}
      ...
      {activeTab === "attendance" ? (
        <div className={styles.classroomHubContent}>
          <AttendanceBoard 
            ...
            students={activeEnrollments.map((e: any) => e.student)} 
          />
        </div>
      ) : null}
